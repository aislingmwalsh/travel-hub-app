              ...
              <div className="min-w-0 flex-1">
                 ...
                 <div className="flex items-center gap-2 flex-wrap mb-1">
                   <span 
                     ...
                     title={item.isAutoAdjusted ? `Auto-adjusted for arrival & travel time (Standard check-in: ${item.baseCheckInTime})` : undefined}
                   >
                     <Clock className={`w-3 h-3 ${item.isAutoAdjusted ? 'text-amber-600' : 'text-blue-600'}`} />{item.time}
                   </span>
                   {item.arrivalTime && (
                   ...
                   {!isLuggageDrop && <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full border ${badgeClass}`}>{item.category || 'Other'}</span>}
